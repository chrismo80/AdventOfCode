...
public static class Day19
{
	...
	public static void Solve()
	{
		...
		var checks = designs.AsParallel().Select(design =>
			design.CountWays(towels.Where(t => design.Contains(t)).ToArray()));
	...
	{
		// remaining is empty, so fully doable, add one more way
		if (remaining.Length == 0)
			return 1;

		// if already calculated just return value
		if (_cache.TryGetValue(remaining, out var count))
			return count;

		// not yet calculated, do so and store in cache
