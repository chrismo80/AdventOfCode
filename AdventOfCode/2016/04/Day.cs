...
public static class Day4
{
	public static IEnumerable<object> Solve(string input)
	{
		...
		yield return data.Where(room => Check(room.Name) == room.Checksum).Sum(room => room.SectorId);
		yield return data.First(room => Shift(room.Name, room.SectorId) == "northpole object storage").SectorId;

		static string Check(IEnumerable<string> name)
		{
		...
		{
			return string.Join(' ', name.Select(word =>
