...
public static class Day19
{
	public static IEnumerable<object> Solve(string input)
	{
		...
		while (result == 0)
		{
			...
			droneSystem.Outputs.TryTake(out var value, 1000);
		...
		void Print()
		{
			...
			for (var y = beam.Min(w => w.Y); y <= beam.Max(w => w.Y); y++)
			{
				var row = new List<char>();

				for (var x = beam.Min(w => w.X); x <= beam.Max(w => w.X); x++)
