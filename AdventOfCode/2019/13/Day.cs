...
public static class Day13
{
	public static IEnumerable<object> Solve(string input)
	{
		...
		yield return screen.Count(tile => tile.Id == 2);
		...
		{
			game.Outputs.TryTake(out var X, 100);
			game.Outputs.TryTake(out var Y, 100);
			game.Outputs.TryTake(out var Id, 100);
	...
	private static void Print(int[][] grid, string file, int sleep = 0)
	{
		Thread.Sleep(sleep);
