...
public static class Day17
{
    ...
    {
        var program = File.ReadAllLines("AdventOfCode/2019/17/Input.txt")[0]
        ...
        while (robot.Running)
        {
            robot.Outputs.TryTake(out int value, 1000);
            Console.Write((char)value);

            switch (value)
            {
                case 10: X = -1; Y++; break;
                case 35: if (Y < 50) { scaffold.Add((X, Y)); } break;
            }
            X++;
        }

        Console.WriteLine(scaffold.Where(location => IsIntersection(location)).Sum(i => i.X * i.Y));
        Console.WriteLine(robot.Outputs.Last());
