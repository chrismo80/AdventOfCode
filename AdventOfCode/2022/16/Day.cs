namespace AdventOfCode2022
{
    public static class Day16
    {
        ...
        {
            var input = File.ReadAllLines("AdventOfCode/2022/16/Test.txt")
            ...
            input.ToList().ForEach(valve => valve.Tunnels.ToList().ForEach(tunnel =>
                graph.Add((valve.Name, tunnel))));

            List<Valve> Valves = new(), Path = new(), Opened = new(), Check = new();

            foreach (var (Name, Flowrate, Tunnels) in input)
                Valves.Add(new Valve() { Name = Name, Flowrate = Flowrate });

            var result1 = BestPath("AA").ToArray();
        ...
        {
            do { yield return current; current = current.Parents.Pop()!; } while (current != null);
        }
