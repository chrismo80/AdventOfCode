...
public static class Day12
{
    public static void Solve()
    {
        ...
        var search = new PathFinding.Grid<char>() { Map = map, Walkable = (next, current) => next - current <= 1 };
        ...
        {
            for (int col = 0; col < map[0].Length; col++)
                if (map[row][col] == 'a') distances[(col, row)] = int.MaxValue;
        }
        ...
        Parallel.ForEach(distances.Keys, start =>
        {
            ...
            {
                Map = map,
                Walkable = (next, current) => next - current <= 1
            };
