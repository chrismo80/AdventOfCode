...
public static class Day17
{
    ...
    {
        var input = File.ReadAllLines("AdventOfCode/2022/17/Input.txt")[0];
        ...
        for (int i = 0; i < 2022; i++) //(1000000000000 % 1745); i++)
        {
        ...
        Console.WriteLine(stopped.Max(s => s.Y));
        Console.WriteLine(stopped.Max(s => s.Y) + ((1000000000000 / 1745) * 2778));
    }

    static HashSet<(int X, int Y)> Left(this HashSet<(int X, int Y)> rock) => Move(rock, -1, 0);
    static HashSet<(int X, int Y)> Right(this HashSet<(int X, int Y)> rock) => Move(rock, 1, 0);
    static HashSet<(int X, int Y)> Down(this HashSet<(int X, int Y)> rock) => Move(rock, 0, -1);
    static HashSet<(int X, int Y)> Up(this HashSet<(int X, int Y)> rock) => Move(rock, 0, 1);

    static HashSet<(int X, int Y)> Move(HashSet<(int X, int Y)> rock, int x, int y) =>
        rock.Select(r => (r.X + x, r.Y + y)).ToHashSet();

    static HashSet<(int X, int Y)> NewShape(int type = 0, int y = 0) =>
    ...
    static void PrintMap(HashSet<(int X, int Y)> stopped, HashSet<(int X, int Y)> falling)
    {
        Thread.Sleep(100);
        var output = new List<string>();

        for (int y = stopped.Max(w => w.Y) + 10; y >= stopped.Min(w => w.Y); y--)
        {
            var row = new List<char>();

            for (int x = stopped.Min(w => w.X); x <= stopped.Max(w => w.X); x++)
