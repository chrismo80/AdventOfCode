namespace AdventOfCode2017
{
    public static class Day21
    {
        ...
        static string FlipH(this string pattern) =>
            string.Join('/', pattern.Split('/').Select(p => new string(p.Reverse().ToArray())));

        static string FlipV(this string pattern) =>
            string.Join('/', pattern.Split('/').Reverse());

        static string Rot90(this string pattern)
        {
            var grid = pattern.Split('/').Select(c => c.ToArray()).ToArray();
            var rot = pattern.Split('/').Select(c => c.ToArray()).ToArray();

            for (int i = 0; i < grid.Length; ++i)
                for (int j = 0; j < grid.Length; ++j) rot[i][j] = grid[grid.Length - j - 1][i];
