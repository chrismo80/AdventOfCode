...
public static class Day23
{
    ...
    {
        var program = File.ReadAllLines("AdventOfCode/2017/23/Input.txt");

        Console.WriteLine($"Part 1: {Run(0)}, Part 1: {Run(0)}");
        ...
        long Run(int start = 0)
        {
            ...
            while (i < program.Length && c < 100_000_000)
            {
                ...
                switch (cmd[0])
                {
                    case "set": registers[cmd[1][0]] = value; i++; break;
                    case "sub": registers[cmd[1][0]] -= value; i++; break;
                    case "mul": registers[cmd[1][0]] *= value; i++; break;
