...
public static class Day17
{
	public static IEnumerable<object> Solve(string input)
	{
		...
		(int X, int Y) LeftOf((int X, int Y) drop)
		{
			return (drop.X - 1, drop.Y);
		}

		(int X, int Y) RightOf((int X, int Y) drop)
		{
			return (drop.X + 1, drop.Y);
		}

		(int X, int Y) DownOf((int X, int Y) drop)
		{
			return (drop.X, drop.Y + 1);
		}

		bool Occupied((int X, int Y) drop)
		{
			return walls.Contains(drop) || water.Contains(drop);
		}
		...
		yield return water.Count;

		PrintMap(walls, water);
	}

	private static void PrintMap(HashSet<(int X, int Y)> walls, HashSet<(int X, int Y)> water)
	{
		Thread.Sleep(100);
		...
		Output.Save(2018, 17, string.Join(Environment.NewLine, output));
	}
}